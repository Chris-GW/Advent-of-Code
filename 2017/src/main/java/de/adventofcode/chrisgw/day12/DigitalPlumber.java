...
public class DigitalPlumber {
    ...
    public static final Pattern PROGRAM_LINE_PATTERN = Pattern.compile("(\\d+)\\s+<->\\s+((\\d+,?\\s*)+)");

    public static Map<Integer, DigitalProgram> parseDigitalProgramConnections(List<String> programLines) {
        Map<Integer, DigitalProgram> idToProgramMap = new HashMap<>();

        for (String programLine : programLines) {
            Matcher programMatcher = PROGRAM_LINE_PATTERN.matcher(programLine);
            if (!programMatcher.matches()) {
                throw new IllegalArgumentException(
                        String.format("Program line doesn't follow expected pattern '%s': %s",
                                PROGRAM_LINE_PATTERN.pattern(), programLine));
            }

            int programId = Integer.parseInt(programMatcher.group(1));
            DigitalProgram program = idToProgramMap.computeIfAbsent(programId, DigitalProgram::new);

            String[] splittedConnectedPrograms = programMatcher.group(2).split(",");
            for (String connectedProgramIdStr : splittedConnectedPrograms) {
                Integer connectedProgramId = Integer.valueOf(connectedProgramIdStr.trim());
                DigitalProgram connectedProgram = idToProgramMap.computeIfAbsent(connectedProgramId,
                        DigitalProgram::new);
                program.addConnectedDigitalProgam(connectedProgram);
            }
        }

        return idToProgramMap;
    }


    public static int countConnectedDigitalProgramGroups(Map<Integer, DigitalProgram> idToProgramMap) {
        int programGroups = 0;
        Set<DigitalProgram> totalDigitalPrograms = new HashSet<>(idToProgramMap.values());
        for (DigitalProgram program : idToProgramMap.values()) {
            if (!totalDigitalPrograms.contains(program)) {
                continue;
            }
            totalDigitalPrograms.removeAll(program.getAllReachableDigitalPrograms());
            programGroups++;
        }
