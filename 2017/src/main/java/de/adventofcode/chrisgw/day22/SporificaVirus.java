...
public class SporificaVirus {
    ...
    protected int x = 0;
    protected int y = 0;
    protected VirusDirection direction = VirusDirection.UP;

    protected GridComputingCluster gridComputingCluster;
    ...
    public void nextVirusBurst() {
        ComputingNodeStatus currentNodeStatus = gridComputingCluster.getNodeStatus(x, y);
        turnVirusDirection(currentNodeStatus);
        modifyCurrenNode(currentNodeStatus);
        moveForwardInDirection();
    }


    protected void turnVirusDirection(ComputingNodeStatus currentNodeStatus) {
        if (ComputingNodeStatus.INFECTED.equals(currentNodeStatus)) {
    ...
    protected void moveForwardInDirection() {
        switch (direction) {
        case UP:
            y++;
            break;
    ...
    public String toString() {
        ...
        for (int y = 4; y >= -4; y--) {
            for (int x = -4; x <= 4; x++) {
                ComputingNodeStatus nodeStatus = gridComputingCluster.getNodeStatus(x, y);
    ...
    protected enum VirusDirection {
        ...
        public VirusDirection getReverseDirection() {
            return this.getDirectionToRight().getDirectionToRight();
        }
