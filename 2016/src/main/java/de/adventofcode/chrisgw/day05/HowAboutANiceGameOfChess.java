 ...
 * --- Part Two ---
 *
 * As the door slides open, you are presented with asecond door that uses a
 ...
 * password. Use only the first result for each position, and ignore invalid positions.
 *
 * For example, if the Door ID is abc:
 *
...
public class HowAboutANiceGameOfChess {
    ...
    public static String generateSecurePassword(String doorId) throws NoSuchAlgorithmException {
    ...
    public static String generateBetterSecurePassword(String doorId) throws NoSuchAlgorithmException {
        int passwordLength = 8;
        StringBuilder password = new StringBuilder(passwordLength);
        password.setLength(passwordLength);
        MessageDigest md = MessageDigest.getInstance("MD5");

        for (int i = 0, k = 0; k < passwordLength; i++) {
            String nextPassword = doorId + i;
            md.update(nextPassword.getBytes());
            byte[] digest = md.digest();
            String passwordHashHexadecimal = DatatypeConverter.printHexBinary(digest);
            ...
            if (isValidPasswordHash(passwordHashHexadecimal)) {
                char positionLetter = passwordHashHexadecimal.charAt(5);
                char newPasswordLetter = passwordHashHexadecimal.charAt(6);
                int position = positionLetter - '0';

                if (isValidPasswordPostion(positionLetter) && password.charAt(position) == '\u0000') {
                    password.setCharAt(position, newPasswordLetter);
                    k++;
                }
            }
        }
    ...
    private static boolean isValidPasswordPostion(char position) {
